...
const RegionalSummaryCard = ({ 
  ...
  const defaultCriticalZones = [
    { name: "3 zonas críticas", description: "vegetação severamente degradada", area: "4,2 ha", color: "red" },
  ];
  
  const defaultAttentionZones = [
    { name: "2 zonas necessitam monitoramento intensivo", area: "3,8 ha", color: "orange" },
  ];
  
  const defaultPreventiveZones = [
    { name: "3 zonas requerem ações preventivas", area: "4,7 ha", color: "yellow" },
  ];

  const zones = {
    critical: criticalZones.length > 0 ? criticalZones : defaultCriticalZones,
    attention: attentionZones.length > 0 ? attentionZones : defaultAttentionZones,
    preventive: preventiveZones.length > 0 ? preventiveZones : defaultPreventiveZones,
  };
      ...
      <div className="relative z-10">
        ...
        >
          <h3 className="text-lg font-bold text-white mb-1">
            Resumo da <span className="text-amber-400">situação</span> regional
          </h3>
          <p className="text-sm text-amber-100/80">
            {regionName} apresenta desafios ambientais que requerem ação imediata da comunidade.
          </p>
        </motion.div>
