...
import { Check, BarChart3, Activity, Leaf, Camera, TrendingUp, TrendingDown } from "lucide-react";
import {
  NDVITooltip,
  DegradationTooltip,
  AreaTooltip,
...
const ZoneCard = ({
  ...
  const { id, name, priority, area, ndvi, degradation, color, latitude, longitude } = zoneData;
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-xl border ${
        colors.border
      } bg-slate-800/60 p-4 sm:p-6 shadow-xl backdrop-blur-xl transition-all duration-300 ease-in-out ${
        colors.shadow
      } cursor-pointer hover:-translate-y-2 hover:bg-slate-800/80 ${
      ...
      <div className="relative z-10">
        <div className="mb-3 sm:mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            >
              {isChecked && <Check className="h-2.5 w-2.5 sm:h-3 sm:w-3" />}
            </button>
            <div>
        ...
        <div className="space-y-2 sm:space-y-3 text-xs sm:text-sm text-slate-300">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className={`font-medium ${colors.text}`}>{degradation}</span>
          </div>
          <div className="flex justify-between items-center">
            <span>Coordenadas:</span>
            <span className="text-slate-300 font-mono">{latitude}, {longitude}</span>
          </div>
        </div>
        ...
        <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-slate-700">
          <div className="flex items-center justify-between mb-2">
            <span className="text-slate-300 text-xs sm:text-sm font-medium">Evolução NDVI</span>
            <span className="text-slate-400 text-xs">Últimos 30 dias</span>
          </div>
          <div className="space-y-2">
            <div className="flex items-center justify-between">
               <span className="text-slate-400 text-xs">Último registro:</span>
               <span className={`text-xs font-medium ${
                 parseFloat(ndvi) >= 0.6 ? 'text-green-400' :
                 parseFloat(ndvi) >= 0.4 ? 'text-yellow-400' :
                 parseFloat(ndvi) >= 0.2 ? 'text-orange-400' :
                 'text-red-400'
               }`}>{(parseFloat(ndvi) - 0.02).toFixed(2)}</span>
             </div>
            <div className="flex items-center justify-between">
        ...
        <div className={`mt-3 sm:mt-4 border-t ${colors.divider} pt-3 sm:pt-4`}>
          <div className="flex items-center justify-between text-xs">
            ...
            <button
              ...
              className={`flex items-center gap-1 px-2 py-1 bg-slate-700/50 hover:bg-slate-600/50 rounded-md text-slate-400 ${colors.hover} transition-colors cursor-pointer`}
            >
