...
const NDVIMap = ({
   ...
   // Atualiza/Desenha AOI GeoJSON quando fornecido
   useEffect(() => {
      if (!map.current || !mapLoaded || !aoiGeoJSON) return;
      ...
      try {
         ...
         if (map.current.getSource("aoi-geojson"))
            map.current.removeSource("aoi-geojson");

         map.current.addSource("aoi-geojson", {
            type: "geojson",
            data: aoiGeoJSON,
         });
         ...
         // Fit bounds ao AOI
         const bbox = getGeoJSONBounds(aoiGeoJSON);
         if (bbox) {
            map.current.fitBounds(bbox, { padding: 24, duration: 700 });
         }
      } catch (e) {
         console.error("Erro ao desenhar AOI:", e);
      }
   }, [aoiGeoJSON, mapLoaded]);
      ...
      try {
         ...
         const visit = (g) => {
            ...
            if (t === "FeatureCollection")
               g.features.forEach((f) => visit(f.geometry));
            else if (t === "Feature") visit(g.geometry);
            else if (t === "Point") coords.push(g.coordinates);
               ...
               g.geometries.forEach((gg) => visit(gg));
         };
         visit(geojson);
